module timer(
    ...
    reg [13:0]i;//be used for creating timer_out
    reg [13:0]fin_timer;//last 10seconds timer
    ...
    always @ (posedge clk_N) begin//mode change right now
        if(!rst)begin
            //set time here in state 'Start', will initial the timer_set
            if(power_in==1'b1 && power_out==1'b0 && state==4'b0000) begin
                // water_line=0;//will reset the water line
                fin_timer = 64*10;
                i=0;
                if(status_mode==3'b001) begin//xi_piao_tuo
                    ...
                    timer_set = 64*(status_weight+2+3);
                end
                else timer_set = 0;
            end

            //wait for the work clock, count 'timer' and 'water_line'
            else if(power_in==1'b1 && power_out==1'b1)begin
                fin_timer = 64*10;
                i=i+1;
                //timer_set--
            ...
            //timer in state FINISH, count 'fin_timer'
            else if(power_in==1'b0 && power_out==1'b1) begin
            ...
            //create next_fin
            if(fin_timer==64*1) next_fin = 1'b1;
            else next_fin = 0;

            //create timer_out to switch FSM
            case(status_mode)
                //x_p_t
