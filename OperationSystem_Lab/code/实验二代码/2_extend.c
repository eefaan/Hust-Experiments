...
int main(void)
{
	...
	// 创建四个线程 pthread_create(pthread_t *thread, const pthread_attr_t *attr, void *(*start_routine)(void *), void *arg);
	...
	// 等待四个线程运行结束 pthread_join(pthread_t thread, void **value_ptr);
    int status;
