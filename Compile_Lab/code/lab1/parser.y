...
%token PLUS MINUS STAR DIV ASSIGNOP AND OR NOT IF ELSE WHILE RETURN BREAK
...
Dec:     VarDec  {$$=$1;}
	   ...
	   | VarDec ASSIGNPLUS Exp  {$$=mknode(ASSIGNPLUS,$1,$3,NULL,yylineno);strcpy($$->type_id,"ASSIGNPLUS");}
	   | VarDec ASSIGNMINUS Exp  {$$=mknode(ASSIGNMINUS,$1,$3,NULL,yylineno);strcpy($$->type_id,"ASSIGNMINUS");}
	   | VarDec ASSIGNSTAR Exp  {$$=mknode(ASSIGNSTAR,$1,$3,NULL,yylineno);strcpy($$->type_id,"ASSIGNSTAR");}
	   | VarDec ASSIGNDIV Exp  {$$=mknode(ASSIGNDIV,$1,$3,NULL,yylineno);strcpy($$->type_id,"ASSIGNDIV");}
       ;
Exp:    Exp ASSIGNOP Exp {$$=mknode(ASSIGNOP,$1,$3,NULL,yylineno);strcpy($$->type_id,"ASSIGNOP");}//$$结点type_id空置未用，正好存放运算符
	  | Exp ASSIGNPLUS Exp {$$=mknode(ASSIGNPLUS,$1,$3,NULL,yylineno);strcpy($$->type_id,"ASSIGNPLUS");}
	  | Exp ASSIGNMINUS Exp {$$=mknode(ASSIGNMINUS,$1,$3,NULL,yylineno);strcpy($$->type_id,"ASSIGNMINUS");}
	  | Exp ASSIGNSTAR Exp {$$=mknode(ASSIGNSTAR,$1,$3,NULL,yylineno);strcpy($$->type_id,"ASSIGNSTAR");}
	  | Exp ASSIGNDIV Exp {$$=mknode(ASSIGNDIV,$1,$3,NULL,yylineno);strcpy($$->type_id,"ASSIGNDIV");}
	  ...
	  | SELFADD Exp %prec FSELFADD {$$=mknode(FSELFADD,$2,NULL,NULL,yylineno);strcpy($$->type_id,"FSELFADD");}
  	  | Exp SELFADD	  {$$=mknode(BSELFADD,$1,NULL,NULL,yylineno);strcpy($$->type_id,"BSELFADD");}
	  | SELFDEC Exp %prec FSELFDEC {$$=mknode(FSELFDEC,$2,NULL,NULL,yylineno);strcpy($$->type_id,"FSELFDEC");}
	  | Exp SELFDEC	  {$$=mknode(BSELFDEC,$1,NULL,NULL,yylineno);strcpy($$->type_id,"BSELFDEC");}
