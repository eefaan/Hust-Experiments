...
void prnTAR(struct codenode *head){
    ...
    do {
		...
		char temp_v2[32] = "$";
		char temp_v3[32] = "$";
		if(strstr(resultstr,"temp")!=NULL){resultstr[5]='\0';strcpy(resultstr, strcat(temp_t1,resultstr+4));}
		else if(resultstr[0]=='v'){resultstr[0]='s';strcpy(resultstr, strcat(temp_v1,resultstr));if(resultstr[2]=='8')resultstr[2]='0';}
		if(strstr(opnstr1,"temp")!=NULL){opnstr1[5]='\0';strcpy(opnstr1, strcat(temp_t2,opnstr1+4));}
		else if(opnstr1[0]=='v'){opnstr1[0]='s';strcpy(opnstr1, strcat(temp_v2,opnstr1));if(opnstr1[2]=='8')opnstr1[2]='0';}
		if(strstr(opnstr2,"temp")!=NULL){opnstr2[5]='\0';strcpy(opnstr2, strcat(temp_t3,opnstr2+4));}
		else if(opnstr2[0]=='v'){opnstr2[0]='s';strcpy(opnstr2, strcat(temp_v3,opnstr2));if(opnstr2[2]=='8')opnstr2[2]='0';}
		...
		switch (h->op) {
            case ASSIGNOP:  //printf("  %s := %s\n",resultstr,opnstr1);
							if(opnstr1[0] == '#')printf("  addi %s,$zero,%s\n",resultstr,opnstr1+1);
            ...
            case PARAM:    //printf("  PARAM %s\n",h->result.id);
						   ...
						   if(strstr(h->result.id,"temp")!=NULL){strcpy(para, strcat(temp_t4,h->result.id+4));para[3]='\0';}
		                   else if(h->result.id[0]=='v'){strcpy(para, strcat(temp_v4,h->result.id+1));para[3]='\0';}
...
int  match_param(int i,struct node *T){
    ...
    for (j=1;j<=num;j++) {
        if (!T){
            semantic_error(pos_temp,"", "函数调用参数太少");
            return 0;
            }
...
{//处理基本表达式，参考文献[2]p82的思想
  ...
  if (T)
	...
	case BSELFADD:
				if (T->ptr[0]->kind!=ID){
					semantic_error(T->pos,"", "自增语句需要对左值进行操作");
					}
				break;
	case FSELFDEC:
	case BSELFDEC:
				if (T->ptr[0]->kind!=ID){
					semantic_error(T->pos,"", "自减语句需要对左值进行操作");
					}
				break;
	case BREAK:
				if(while_en==0)semantic_error(T->pos,T->type_id, "在循环体外不应使用break");
				break;
	case CONTINUE:
				if(while_en==0)semantic_error(T->pos,T->type_id, "在循环体外不应使用continue");
				break;
	case ID:    //查符号表，获得符号表中的位置，类型送type
                rtn=searchSymbolTable(T->type_id);
                if (rtn==-1)
