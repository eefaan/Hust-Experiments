...
void subp_writebuf()
{
	...
	if ((fd=open(src_file_name,O_RDONLY))==-1){
...
void subp_readbuf()
{
	...
	struct buffer_node *temp=head;
	if ((fd=open(tar_file_name,O_WRONLY|O_CREAT,S_IRWXU|S_IXGRP|S_IROTH|S_IXOTH))==-1){
		printf("open file fail\n");
		return;
	}
	else
...
{
    if(argc != 3){
        printf("Usage: copy <源文件名> <目标文件名>\n");
        return 0;
    }
    // get file name
    strcpy(src_file_name, argv[1]);
    strcpy(tar_file_name, argv[2]);
