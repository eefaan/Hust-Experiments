#include "buyer_mw.h"
#include "ui_buyer_mw.h"

buyer_mw::buyer_mw(QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::buyer_mw)
{
    ui->setupUi(this);
...
void buyer_mw::on_logout_clicked(){
    emit logout_signal();
    return;
}

void buyer_mw::on_flash_clicked(){
    emit flash_signal();
    return;
}

void buyer_mw::on_submit_clicked(){
    QString request;
    request = "3,"+ui->gname->text()+","+ui->gplace->text()+","+ui->gmanu->text()+","+ui->gnum->text();
    emit submit_signal(request);
    return;
}

void buyer_mw::on_gflash_clicked(){
    emit gflash_signal();
    return;
}

void buyer_mw::proc_response_flash(QString response){
...
void buyer_mw::proc_response_submit(QString response){
    /* response's construct:
...
void buyer_mw::proc_response_gflash(QString response){
    /* response's construct:
     *     response_type, alarm_message, flash_flag, pono, gno, gnum, auti_flag, ano, date
     */
    QString section1 = response.section(",",1,1);// get alarm_message
